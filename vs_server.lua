...
RegisterCommand('freezetime', function(source, args)
	if source ~= 0 then
		if isAllowedToChange(source) then
			...
			if freezeTime then
				TriggerClientEvent('es_wsync:notify', source, 'Time is now ~b~frozen~s~.')
			else
				TriggerClientEvent('es_wsync:notify', source, 'Time is ~y~no longer frozen~s~.')
			end
		else
	...
	if source ~= 0 then
		if isAllowedToChange(source) then
			...
			if not DynamicWeather then
				TriggerClientEvent('es_wsync:notify', source, 'Dynamic weather changes are now ~r~disabled~s~.')
			else
				TriggerClientEvent('es_wsync:notify', source, 'Dynamic weather changes are now ~b~enabled~s~.')
			end
		else
			TriggerClientEvent('chatMessage', source, '', {255,255,255}, '^8Error: ^1You are not allowed to use this command.')
		end
	...
	if source == 0 then
		...
		if isAllowedToChange(source) then
			...
			if args[1] == nil then
				...
				if validWeatherType then
					TriggerClientEvent('es_wsync:notify', source, 'Weather will change to: ~y~' .. string.lower(args[1]) .. "~s~.")
					CurrentWeather = string.upper(args[1])
					newWeatherTimer = 10
					TriggerEvent('es_wsync:requestSync')
				else
...
RegisterCommand('blackout', function(source)
	if source == 0 then
		...
		if isAllowedToChange(source) then
			blackout = not blackout
			if blackout then
				TriggerClientEvent('es_wsync:notify', source, 'Blackout is now ~b~enabled~s~.')
			else
				TriggerClientEvent('es_wsync:notify', source, 'Blackout is now ~r~disabled~s~.')
			end
			TriggerEvent('es_wsync:requestSync')
		end
	end
...
RegisterCommand('morning', function(source)
	...
	if isAllowedToChange(source) then
		ShiftToMinute(0)
		ShiftToHour(9)
		TriggerClientEvent('es_wsync:notify', source, 'Time set to ~y~morning~s~.')
		TriggerEvent('es_wsync:requestSync')
	end
end)
RegisterCommand('noon', function(source)
	...
	if isAllowedToChange(source) then
		ShiftToMinute(0)
		ShiftToHour(12)
		TriggerClientEvent('es_wsync:notify', source, 'Time set to ~y~noon~s~.')
		TriggerEvent('es_wsync:requestSync')
	end
end)
RegisterCommand('evening', function(source)
	...
	if isAllowedToChange(source) then
		ShiftToMinute(0)
		ShiftToHour(18)
		TriggerClientEvent('es_wsync:notify', source, 'Time set to ~y~evening~s~.')
		TriggerEvent('es_wsync:requestSync')
	end
end)
RegisterCommand('night', function(source)
	...
	if isAllowedToChange(source) then
		ShiftToMinute(0)
		ShiftToHour(23)
		TriggerClientEvent('es_wsync:notify', source, 'Time set to ~y~night~s~.')
		TriggerEvent('es_wsync:requestSync')
	end
end)
	...
	elseif source ~= 0 then
		if isAllowedToChange(source) then
			if tonumber(args[1]) ~= nil and tonumber(args[2]) ~= nil then
				...
				TriggerClientEvent('es_wsync:notify', source, 'Time was changed to: ~y~' .. newtime .. "~s~!")
				TriggerEvent('es_wsync:requestSync')
			else
